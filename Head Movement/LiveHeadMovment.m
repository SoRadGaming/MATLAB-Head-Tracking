...
%Optimiser
result = zeros(1000,3,'double');
orin = zeros(1,1000,'quaternion');
...
while (toc < 30) %run for 30 secs
      ...
      [a, ~] = accellog(AndroidPhone);
      [w, ~] = angvellog(AndroidPhone);
      ...
      if lp < lim
      % Redraw plot
      rotate(turtle,[1,0,0],((result(lp,2)*180/pi) - (result(lp - 1,2)*180/pi))) %Pitch
      rotate(turtle,[0,1,0],((result(lp,3)*180/pi) - (result(lp - 1,3)*180/pi))) %Roll
      rotate(turtle,[0,0,1],(((result(lp,1)*180/pi) - (result(lp - 1,1)*180/pi)))) %Yaw
      drawnow
      view(3)
      lp = lp + 1;
